 ...
 * 		  2) 상어 이동
 * 		  -> R*C 전체 탐색
 * 		  -> 상어 만나면 이동, 최대 R*2-1만큼 이동
 * 	
 * 	  - 최적화
 * 		* 시간  
 * 		  1) 속력(s) 그대로 한칸씩 이동 -> 2552ms
 * 		  2) 속력(s)이 R*2-1인 경우, 처음 방향을 그대로 유지하면서 제자리로 돌아온다. (열 이동인 경우, C*2-1)
 * 			 이러한 논리를 토대로, s%=(R*2-1)으로 이동 횟수를 줄임 -> 824ms
 * 
 * 		* 메모리
...
public class Main_ja {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < M; i++) {
			...
			map[r][c].s %= (((map[r][c].d == 1 || map[r][c].d == 2) ? R : C) * 2 - 2); 
